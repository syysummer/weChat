            ...
            <img v-lazy="used_en" v-if="language == 'en'"> -->
				...
				<div class="coupon_items" :class="item.universal_id == 0 ? 'coupon_platform' : 'coupon_store'">
					...
					<div class="coupon_detail">
						<p class="coupon_name" v-show="item.fullMoney" :class="item.universal_id == 0 ? '' : 'active'">{{en_txt.full}} {{item.fullMoney}} {{en_txt.to_use}}</p>
						<p class="coupon_name" v-show="!item.fullMoney" :class="item.universal_id == 0 ? '' : 'active'">{{en_txt.no_limit}}</p>
						<p class="expired_date" :class="{store_color: item.universal_id == 1, iphone5: isIphone5}">{{$formatDateTime(item.startTime, 'ymd').replace(/-/g,'.')}}-{{$formatDateTime(item.expireTime, 'ymd').replace(/-/g,'.')}}</p>
						<p class="use_platform" v-if="item.universal_id == 0 && item.sourceFlag == 1">{{en_txt.source}}：{{en_txt.platform_coupons}}</p>
						<p class="use_platform" v-if="item.universal_id == 0 && item.sourceFlag == 3">{{en_txt.source}}：{{en_txt.invites_prizes}}</p>
						<p class="use_platform" v-if="item.universal_id == 1">{{en_txt.source}}：{{en_txt.merchant_coupons}}</p>
						<p class="use_platform act" v-if="item.universal_id == 0 && item.sellerCatId == 0">{{en_txt.platform_use}}</p>
						<p class="use_platform act" v-if="item.universal_id == 0 && item.sellerCatId != 0">{{en_txt.limit}}{{item.name}}{{en_txt.category}}</p>
						<p class="use_platform act" v-if="item.universal_id == 1">{{en_txt.limit}} {{item.sellerName}} {{en_txt.use}}</p>
					...
					<div v-if="item.no_use == 1" class="use-button">
						...
						<div class="coupon_right" :class="item.universal_id == 1 ? 'act' : ''" v-else>
							<div @click="goUse(item.universal_id,item.sellerCatId,item.sellerId,item.name)">{{en_txt.go_use}}</div>
						</div>
					</div>
				</div>
...
export default {
  ...
  methods: {
    goUse(universal_id, sellerCatId = '', sellerId = '', name = '') {
      if (universal_id == 0) {
